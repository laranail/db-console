...
/**
 * Detach one user to/from many databases in a single batch (repeatable --db).
 */
final class DetachCommand extends DBConsoleCommand
{
    protected $signature = 'laranail::db-console.detach {--server=} {--user=} {--host=} {--db=*} {--preset=}';
    ...
    public function handle(PrivilegeManager $privileges): int
    {
        ...
        try {
            ...
            $result = $privileges->detach($this->server(), new Username($user), new Host($host), $databases, $set);
