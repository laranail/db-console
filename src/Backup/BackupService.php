 ...
 * (section 7). Backed by laranail/db-tools when present; when absent
...
interface BackupService
{
     ...
     * disabled or unavailable (db-tools not installed).
     */
    public function snapshot(string $server, DbName $database): ?string;

    /**
     * Whether backups can actually run (config enabled AND db-tools
